...
- [ ] **INFRA-05**: Development environment orchestrates all processes via concurrently
...
- [ ] **PERS-01**: Admin can create a personnel record with name, custom ID, person type (allow/block), and photo
- [ ] **PERS-02**: Admin can edit personnel details and replace photo
- [ ] **PERS-03**: Admin can delete a personnel record (propagates delete to all cameras)
- [ ] **PERS-04**: Personnel list page shows all personnel with avatar, name, custom ID, list type, and sync status dot
- [ ] **PERS-05**: Personnel detail page shows edit form (left) and per-camera enrollment status sidebar (right)
- [ ] **PERS-06**: Photo upload uses a dropzone with client-side preview and displays size constraints as help text
- [ ] **PERS-07**: System preprocesses photos: resize to max 1080p, compress to JPEG <1MB, compute MD5 hash
- [ ] **PERS-08**: Sync status dot on personnel list summarizes camera enrollment: green (all enrolled), amber (pending), red (failed)

### Enrollment Sync

- [ ] **ENRL-01**: Saving a personnel record dispatches enrollment jobs to all cameras via MQTT EditPersonsNew
- [ ] **ENRL-02**: Enrollment batches are limited to 1000 entries; larger sets are chunked
- [ ] **ENRL-03**: Only one enrollment batch may be in-flight per camera at a time (WithoutOverlapping middleware)
